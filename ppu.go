...
func (p *Ppu) renderTileRow() {
	...
	for x := 0; x < 32; x++ {
		...
		p.decodePatternTile([]Word{tile[p.ScanlineCounter], tile[p.ScanlineCounter+8]}, x*8, p.Scanline+p.ScanlineCounter, p.bgPaletteEntry(attr), nil)
	...
	if p.ScanlineCounter == 8 {
		p.ScanlineCounter = 0
	}
}

func (p *Ppu) decodePatternTile(t []Word, x, y int, pal []Word, attr *Word) {
	var b uint
	for b = 0; b < 8; b++ {
		var xcoord int
